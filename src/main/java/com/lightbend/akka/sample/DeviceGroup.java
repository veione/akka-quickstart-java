...
import com.lightbend.akka.sample.Device.TemperatureRecorded;
...
public class DeviceGroup extends AbstractActor {
	 ...
	 * 
	 * <pre>
	 * 1、Temperature：It has a temperature available 2、TemperatureNotAvailable：It
	 * has responded, but has no temperature available yet
	 * 3、DeviceNotAvailable：It has stopped before answering 4、DeviceTimedOut：It
	...
	private void onTrackDevice(DeviceManager.RequestTrackDevice trackMsg) {
		if (this.groupId.equals(trackMsg.groupId)) {
			...
			if (deviceActor != null) {
				...
				getContext().watch(deviceActor);// 监管创建的Device
	...
	private void onAllTemperatures(RequestAllTemperatures r) {
		// since Java collections are mutable, we want to avoid sharing them
		// between actors (since multiple Actors (threads)
		// modifying the same mutable data-structure is not safe), and perform a
		// defensive copy of the mutable map:
		//
		// Feel free to use your favourite immutable data-structures library
		// with Akka in Java applications!
		Map<ActorRef, String> actorToDeviceIdCopy = new HashMap<>(this.actorToDeviceId);

		getContext().actorOf(DeviceGroupQuery.props(actorToDeviceIdCopy, r.requestId, getSelf(),
